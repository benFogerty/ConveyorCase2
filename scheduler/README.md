...
Run Baseline, OrderToteHill, FullHillClimb, SimulatedAnnealing, GeneticAlgorithm, TabuSearch, and IteratedLocalSearch on every instance; all evaluated with **last_order completion**. All outputs go into the **results/** folder.
...
Defaults: 800 evals per joint method, 2 restarts for SA (take best), optional `--polish` (hill climb after SA). Outputs in `results/`: `comparison.csv`, `summary.txt`, `comparison.png`.
...
# Method comparison (Baseline, OrderToteHill, FullHillClimb, SimulatedAnnealing, GeneticAlgorithm, TabuSearch, IteratedLocalSearch) – all outputs in results/
python scheduler/compare_methods.py data_generator --out-dir results
